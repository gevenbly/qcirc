...
function drawMinimap() {
  if (showMini) {
    ...
    var wY1 = roundHP(
      miniHeight * ((windowY0 + windowHeight) / spaceHeight) + mY
    );
    ...
    for (var i = 0; i < tensor_xcoords.length; i++) {
      if (tensor_types[i] == 0) {
        // draw rectangles
        var xc = makeAllInRange(
          allRound(a2mXall(tensor_xcoords[i])),
          mX,
          mX + miniWidth
        );
        var yc = makeAllInRange(
          allRound(a2mYall(tensor_ycoords[i])),
          mY,
          mY + miniHeight
        );
        ...
        var xc = allRound(a2mXall(tensor_xcoords[i]));
        var x0 = Math.min(...xc);
        var xf = Math.max(...xc);
        var xmid = (x0 + xf) / 2;

        var yc = allRound(a2mYall(tensor_ycoords[i]));
        var y0 = Math.min(...yc);
        var yf = Math.max(...yc);
        var ymid = (y0 + yf) / 2;
    ...
    ctxB.fillRect(mX + miniWidth, 0, miniPad, viewHeight);
...
function drawTensors() {
  ...
  for (var i = 0; i < tensor_xcoords.length; i++) {
    ...
    var numAnchors = tensor_xanchors[i].length;
    var xmid = a2rX(tensor_bbox[i][4]);
    var ymid = a2rY(tensor_bbox[i][5]);

    for (var j = 0; j < numAnchors; j++) {
      var colTemp = colourNameToHex(allAnchorColors[j]);
      if (objUnderMouse[0] == "anchor" && objUnderMouse[1] == i && objUnderMouse[2] == j) {
        colTemp = lightenColor(colTemp, 10);
      }
      ctxT.fillStyle = colTemp;
      roundRect(ctxT, 
                tensor_xanchors[i][j] * windowZoom + xmid - anchorWidth/2, 
                tensor_yanchors[i][j] * windowZoom + ymid - anchorHeight/2,
                anchorWidth, anchorHeight, 3);
      ctxT.fill();
      ctxT.stroke()
    }
  }
  ...
  if (stateOfMouse == "selecting") {
    // draw selection box
    // set path
    var x0 = roundHP(a2rX(selectBoxCoords[0]));
    var y0 = roundHP(a2rY(selectBoxCoords[1]));
    var x1 = roundHP(a2rX(selectBoxCoords[2]));
    var y1 = roundHP(a2rY(selectBoxCoords[3]));
...
function drawTensorEllipse(i) {
  // set path
  var x0 = roundHP(a2rX(tensor_bbox[i][0]));
  var y0 = roundHP(a2rY(tensor_bbox[i][1]));
  var xf = roundHP(a2rX(tensor_bbox[i][2]));
  var yf = roundHP(a2rY(tensor_bbox[i][3]));
  var xmid = Math.round(a2rX(tensor_bbox[i][4]));
  var ymid = Math.round(a2rY(tensor_bbox[i][5]));

  ctxT.beginPath();
  ctxT.ellipse(xmid, ymid, (xf - x0) / 2, (yf - y0) / 2, 0, 0, 2 * Math.PI);
  ctxT.stroke();
  ...
  var grd = ctxT.createLinearGradient(
    x0,
    y0,
    x0 + 2 * (xf - x0),
    y0 + 2 * (yf - y0)
  ...
  if (currSelected.includes(i)) {
    if (objUnderMouse[0] == "rename" && objUnderMouse[1] == i) {
      ctxT.drawImage(
        renameIconH,
        Math.round(xmid - renameIcon.width / 2),
        Math.round(ymid + renameIcon.height / 2)
      );
    } else {
      ctxT.drawImage(
        renameIcon,
  ...
  if (currSelected.includes(i)) {
    ctxH.lineWidth = 1;
    ctxH.fillStyle = "#dddddd";
    ctxH.strokeStyle = "#505050";
    var x0 = roundHP(a2rX(tensor_bbox[i][0]));
    var y0 = roundHP(a2rY(tensor_bbox[i][1]));
    var xf = roundHP(a2rX(tensor_bbox[i][2]));
    var yf = roundHP(a2rY(tensor_bbox[i][3]));
    drawCircle(ctxH, x0, (y0 + yf) / 2, circRad, circThick);
    drawCircle(ctxH, (x0 + xf) / 2, y0, circRad, circThick);
    drawCircle(ctxH, xf, (y0 + yf) / 2, circRad, circThick);
    drawCircle(ctxH, (x0 + xf) / 2, yf, circRad, circThick);
  }
}

function drawTensorRectangle(i) {
  // set path
  var x0 = roundHP(a2rX(tensor_bbox[i][0]));
  var y0 = roundHP(a2rY(tensor_bbox[i][1]));
  ...
  // fill
  if (currSelected.includes(i)) {
    if ((objUnderMouse[0] === "tensor" || objUnderMouse[0] === "rename" || objUnderMouse[0] === "anchor") && i == objUnderMouse[1]) {
      var theFillColor = lightenColor("#008f94", 10);
    } else {
      var theFillColor = "#008f94";
    }
